...
const InvoiceList = ({customerId}) => {
    ...
    const handleAllInvoice = async () => {
        try {
            const response = await axios.get(`${apiPath}/invoice/invoiceList?customer=${customerId || ''}`);
    ...
    const confirmDelete = async () => {
        try {
            ...
            const response = await axios.delete(`${apiPath}/invoice/deleteInvoice/${selectedAgent._id}`);
                ...
                <table id="invoice" className="">
                    ...
                    <tbody>
                        {invoiceData && invoiceData.map((item, index) => (
                            <tr key={index} >
                                <td onClick={() => navigate(`/invoice-detail/${item._id}`)} className="border-b"># {item.index}</td>
                                <td onClick={() => navigate(`/invoice-detail/${item._id}`)} className="border-b capitalize">{item.customer?.firstName + " " + item.customer?.lastName}</td>
                                <td onClick={() => navigate(`/invoice-detail/${item._id}`)} className="border-b">{new Date(item.date).toLocaleDateString()}</td>
                                <td onClick={() => navigate(`/invoice-detail/${item._id}`)} className="border-b">$ {item.total}</td>
