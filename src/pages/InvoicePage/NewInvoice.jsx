...
const NewInvoice = () => {
    ...
    // Fetch extra fields whenever financialTemplate changes
    useEffect(() => {
        if (!selectedTemplate) {
            setInputFields([]);
            return;
        ...
        const fetchExtraFields = async () => {
            try {
                ...
                // Autofill custom fields from selected package if present
        ...
        fetchExtraFields();
    }, [selectedTemplate, selectedPackageId, packageList, setValue]);
    ...
    const handlePackageChange = async (packageId) => {
        ...
        if (templateId) {
            ...
            try {
                ...
                // Immediately autofill custom fields from package
                fields.forEach((field) => {
                    const val =
                        selectedPkg.invoice?.[field.name] ??
                        selectedPkg[`invoice_${field.name}`] ??
    ...
    const onSubmit = async (formData) => {
        ...
        try {
            ...
            await axios.post(`${apiPath}/invoice/new_invoice`, finalData);
            toast.success('Invoice created successfully!');
            navigate(-1);
        } catch (error) {
            console.error('Error creating invoice:', error);
            toast.error(
                error.response?.data?.message || error.message || 'Failed to create invoice'
            );
        } finally {
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white dark:bg-boxdark rounded-2xl border border-slate-200/80 dark:border-strokedark p-5 md:p-6 shadow-xs">
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        >
                            ...
                            <span>{isSubmitting ? 'Creating Invoice...' : 'Prepare & Create Invoice'}</span>
                        </button>
                    </div>
                </div>
                            ...
                            <div className="bg-gradient-to-b from-white to-slate-50/80 dark:from-boxdark dark:to-slate-900/60 rounded-2xl border border-slate-200/90 dark:border-strokedark p-6 shadow-md space-y-5 relative">
                                ...
                                <div className="space-y-3 text-xs">
                                    ...
                                    <div className="flex items-center justify-between py-1 border-b border-slate-100/80 dark:border-slate-800/60">
                                        ...
                                        <span
                                            className={`font-bold font-mono text-sm ${discountAmount > 0
