...
const ServicePage: React.FC = () => {
    ...
    const openModal = (group: any) => {
        ...
        if (group) {
            reset({serviceName:group?.serviceName, serviceTypeName: group?.serviceTypeName, price: group?.price, icon: group?.icon });
        }
    };
    ...
    const onSubmit = async (formData: any) => {
        ...
        try {
            if (selectedSalesGroup) {
                await axios.put(`${apiPath}/api/services/${selectedSalesGroup._id}`, formData);
                notify('item updated successfully');
            } else {
                await axios.post(`${apiPath}/api/services`, { ...formData, type: 'service' });
                        ...
                        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6 overflow-auto p-1">
                            <div className="flex flex-col">
                                ...
                                <input
                                    ...
                                    className={`border p-2 rounded focus:outline-none focus:ring focus:ring-blue ${errors.serviceName ? 'border-red-500' : 'border-gray'
                                        }`}
                                />
                                {errors.serviceName && (
                            ...
                            <div className="flex flex-col">
                                ...
                                <input
                                    ...
                                    className={`border p-2 rounded focus:outline-none focus:ring focus:ring-blue ${errors.serviceTypeName ? 'border-red-500' : 'border-gray'
                                        }`}
                                />
                                {/* <select
                                    {...register('serviceTypeName', {
                                ...
                                {errors.serviceTypeName && (
                                    <p className="text-red-500 text-sm mt-1">{errors.serviceTypeName.message}</p>
                                )}
                            </div>
                            ...
                            <div className="flex flex-col">
                                ...
                                <input
                                    ...
                                    className={`border p-2 rounded focus:outline-none focus:ring focus:ring-blue ${errors.price ? 'border-red-500' : 'border-gray'
                                        }`}
                                />
                                {errors.price && (
                ...
                <div className="flex items-center justify-center min-h-screen">
                    <Box className="bg-white p-10 rounded shadow-md max-w-md mx-auto">
                        ...
                        <Typography marginBottom={2} variant='h6'>
                            Are you sure you want to delete the item that named "{selectedSalesGroup?.serviceTypeName}"?
                        </Typography>
