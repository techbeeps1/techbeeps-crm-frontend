...
const Jobslider: React.FC<JobsliderProps> = ({ job, onClose=()=>{}, Ondelete=()=>{} }) => {
  ...
  const [isdeleteBoc, setIsDeleteBox] = useState(String);
...
notifyError("Error while deleting appointment")
  ...
  return (
    <div className="shadow-xl top-0 right-0 h-full left-0 bg-white p-4 overflow-y-auto transition-transform">
      ...
      {tabIndex === 0 && (
        <div className="px-4">
              ...
              <div className="grid grid-cols-2 gap-4 mb-4">
                ...
                <div>
                  ...
                  <p className="font-medium mb-1">
                    {relocation.relocation?.totalVolume} m<sup>3</sup>
                  </p>
                  <p className="font-medium mb-1">
                    {relocation.relocation?.movers} movers
                  </p>
                  <p className="font-medium mb-1">
                    {relocation.total?.handyman} handyman
                  </p>
                  <p className="font-medium mb-1">
                    {relocation.packing?.requiredPackers} Packers
                  </p>
                  <p className="font-medium mb-1">
                    {relocation.unpacking?.requiredPackers} Unpackers
                  </p>
                  <p className="font-medium mb-1">
                    {relocation.relocation?.distance} km{' '}
                  </p>
                </div>
                ...
                <div>
                  ...
                  <p className="font-medium mb-1">
                    {(relocation.relocation?.requiredHours).toFixed(2) ||
                      'Not available'}{' '}
                    hours
                  </p>
                  <p className="font-medium mb-1">
                  ...
                  <p className="font-medium mb-1">
                    {(relocation.assembling?.requiredHours).toFixed(2)}{' '}
                    Assembling hours
                  </p>
                  <p className="font-medium mb-1">
                    {(relocation.disassembling?.requiredHours).toFixed(2)}{' '}
                    Disassembly hours
                  </p>
                  <p className="font-medium mb-1">
                    {(relocation.packing?.requiredHours).toFixed(2)} Packing
                    Hours
                  </p>
                  <p className="font-medium mb-1">
                    {(relocation.unpacking?.requiredHours).toFixed(2)} Unpacking
                    Hours
                  </p>
                </div>
              </div>
          ...
          <div className="mb-4 mt-4">
            <h3 className="font-bold text-lg text-slate-800 mb-3 mt">
              CUSTOMER INFORMATION :
            </h3>

            <div className="grid grid-cols-2 sm:gird-cols-1 gap-5">
              <div className="space-y-1">
                <h2 className="font-medium text-xl pb-2 capitalize">
                  {job?.customer?.type} Details :
                </h2>
                <a
                ...
                {[
                  {
                    label: 'Name',
                    value: `${job?.customer?.salutation || ''} ${job?.customer?.firstName || ''} ${job?.customer?.lastName || ''}`,
                  },
                  ...
                  <div
                    style={{ fontSize: '17px' }}
                    className="flex gap-4 text-slate-600 font-medium"
                    key={label}
                  >
                    <p className="">{label}:</p>
                ...
                job?.customer?.address.map((item: any, index: number) => (
                  <div key={index} className="">
                    ...
                    <div className="flex items-center justify-between">
                      <h2 className="font-bold text-slate-500 capitalize">
                        {item.addressType}
                      </h2>
                    </div>
                    <div
                    ...
                    >
                      <p className="text-slate-800 capitalize max-w-100">
                        {item.floor} Floor {item.houseNumber || '0'}{' '}
                        {item.addition} {item.street} {item.city} {item.country}
                      </p>
                      <p className="text-slate-500">{item.typeOfProperty}</p>
                      <div className="flex gap-2">
                        <span className="relative group">
                          {item.hasElevator && (
                            <ElevatorOutlined fontSize="medium" />
                          )}
                          <div className="absolute buttom-0 rounded w-100 hidden group-hover:flex flex-col gap-2 bg-slate-300 border border-gray p-2 shadow-lg">
                            ...
                            <p>
                              Distance To Apartment :{item.distanceToApartment}m
                            </p>
                          </div>
                        </span>
                        <span>
                          {item.deliveringBoxes && (
                            <GifBoxOutlined fontSize="medium" />
                          )}
                        </span>
                        <span>
                          {item.applyForPermit && (
                            <ApprovalOutlined fontSize="medium" />
                          )}
                        </span>
                      </div>
                    </div>
                  ...
                  <form onSubmit={handleSubmit(handleNotesFormSubmission)}>
                    ...
                    <div className="mb-2">
                      <TextField
                        ...
                        helperText={
                          errors.generalNotes ? errors.generalNotes.message : ''
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
              <div>
                ...
                <Typography variant="body2" className="text-slate-500">
                  Review scheduled appointments for this job.
                </Typography>
              </div>
              <AppointmentScheduler
                allAppointments={getAppointments}
      ...
      {tabIndex === 4 && <DocumentSelected id={job?.customer?._id} />}
      {tabIndex === 5 && <CommunicationLog id={job?._id} />}
      {tabIndex === 6 && (
        <EmailLayout
      ...
      {tabIndex === 7 && <TaskPage jobId={job._id} />}
      {tabIndex === 8 && <QuotesActivity invoiceData={job?.offer[0]} />}
    </div>
  );
};
