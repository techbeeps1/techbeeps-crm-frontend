...
const MultiStepPopup = ({ skills, licenses, handler, countries }) => {
    ...
    const steps = [
        ...
        { label: 'Skills', fields: ['Skill 1', 'Skill 2'] },
    ];
    ...
    if(data.password.trim().length<6  || data.password.trim().length>20){
        ...
        try {
            const response = await axios.post(`${apiPath}/user/register`, transformedData);
            reset();
    ...
    const sendMail = async (detail) => {
        try {
            const response = await axios.post(`${apiPath}/email/send_email`, { emailTemplateId: settings?.emailTemplates?.confirmation, extraData: detail, subject: `hey ${detail.username} Its is a confirmation mail regarding to your work with Techbeeps` });
            if (response.status === 200) {
                notify('Confirmation mail send successfully')
            }
        } catch (error) {
            notifyError(`Error in Sending mail: ${error.message}`);
        }
    };
    ...
    return (
        <>
            ...
            <div className="mt-2">
                <div className='px-3'>
                    <Button variant="contained" className='shadow-md' size='large' onClick={() => setOpen(true)}>New Employee</Button>
                </div>
                <Modal open={open} onClose={handleClose}>
                    <Box className="bg-white p-7 rounded-lg shadow-lg max-w-4xl mx-auto" style={{ maxHeight: "96vh", overflowY: "auto", width: "100%",  position: "absolute",
            top: "50%",
            left: "50%",
